...
public class AccountService {
    ...
    public String importAccoutFromExcel( Integer idClass,@RequestParam("file") MultipartFile multipartFile) {
        try {
            ...
            int mssvColumnIndex = 4;
            int hoColumnIndex = 5;
            int tenColumnIndex =6;
            int lopColumIndex=7;
//            int phoneColumIndex=6;
            ...
            while (rowIterator.hasNext()) {
                ...
                if (row.getRowNum() <1) { // Bỏ qua các dòng trước dòng 7
                    continue;
                }
...
//                    }
                    ...
                    if (accountRepository.findByEmail(user_email) == null) {
                        ...
                        studentRepository.save(new StudentList(idClass, savedAccount.getUserId()));
                        ...
                        if (!studentRepository.existsByClassIdAndStudentId(idClass, userId)) {
                            studentRepository.save(new StudentList(idClass, userId));
                        }
                    }
                }
            }
    ...
    // account all cntt
    public String importAccoutFromExcelAdmin(@RequestParam("file") MultipartFile multipartFile) {
        try {
            // Đọc file Excel
            InputStream inputStream = multipartFile.getInputStream();
            Workbook workbook = new XSSFWorkbook(inputStream);
            ...
            workbook.close();
            inputStream.close();
            return "Dữ liệu đã import thành công";
        } catch (Exception e) {
            e.printStackTrace();
            return "Đã xảy ra lỗi trong quá trình import dữ liệu từ file Excel";
        }
